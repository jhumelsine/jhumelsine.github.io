...
However, the architecture, decomposition, boundaries, communication and judgment by humans is increasing.
...
All I can do is state my opinion for whatever it's worth.
_Caveat Emptor._

The field is changing rapidly.
No one knows how AI will shape software engineering, even those who are using it practically and professionally.

Given my impressions based upon external observations of a constantly changing field, I think the only way I can make progress and publish this is to do the following:
* Structure this blog entry as an anthology of related themes rather than one story. Treat each section as a sort stand alone entry.
* Revisit this entry and update it as my impressions change, there are new advancements and discoveries, and there are new references to add.
...
* Prototypes especially to work out ideas and demo to customers and users. A working model may convey the product better than documents or slides.
* Personal projects, where scalability and security may not be an issue.
* Code generation that's well defined and scoped. For example, adding `equals()` and `hashCode()` to a Java class or even a small well defined class from scratch.

Vibe coding is probably not the best practice for large projects with many users.
...
# Agents
Agents are entities that do tasks on your behalf, often at your request:
...
## Prompts
Prompting is how most people first experience AI such as with ChatGPT.
They interact with it as if in a conversation.
Ask AI a question or give it a task, and it responds.
It will wait indefinitely until you continue.

An AI session tends to be a blank slate until prompts are provided.
Prompts provide context, which will be retained with the session, baring Context Window (TBD) limitations.

My previous [LLM/GenAI](https://jhumelsine.github.io/table-of-contents#ai-and-emerging-topics) blog entries featured this type of interaction, except that I didn't explore engineering prompts in the context of generating code.

I have used this technique to generate code, but it tends to be small well scoped and bounded requests.
I feel like this is mostly [vibe coding](https://jhumelsine.github.io/2026/05/29/code-and-toil.html#vibe-coding-and-the-monkeys-paw).
Vibe code prompting is probably satisfactory for the following:
* Prototype implementations that demonstrate features to customers as the focus of discussions about desired behavior. Prototypes can be used as working models for the development team.
* Small well scoped and bounded problems, as I mentioned above. I use prompting to review the code samples I provide in my blog entries.
* Tools to aid you in your day-to-day work. These will probably be scripts. They will probably a subset of small well scoped and bounded problems.

## Agents
All AI assistants are agents.
Prompting AI platforms, such as ChatGPT, are agents too.

Agents in the context of this blog entry will focus upon writing and confirming softare.
Rather than being conversational, coding agents will design and create code.
They will write test cases.
They will refactor the code.

What they do will be based upon a workflow I suspect not too unlike current software development workflows.
The main distinction will be that agents will take on more of the responsibilities that have previously been the responsibilites of individual contributors.
...
Theses are the same questions we have had with human softare developers as individual contributors.

Though we had testing and other processes to confirm the software, much of the confidence we had in our softare depended upon on the trust we had with experienced developers.
AI is good, but it's not the same looking a human developer in the eye.
AI will also generate much more code at a faster rate than humans can review.
...
For example, it may decide that the best way to get all tests to pass is to remove all of the asserts.
...
This is probably more greatly magnifided with agents.
Agents possess knowledge about coding.
Agents may even posses knowledge about the _wisdome_ topics I listed above.
...
# References
* ["Makers vs Menders" by M. Scott Ford (@mscottford)](https://www.youtube.com/watch?v=YW0BpCThjuM) - Presentation by M. Scott Ford at The Legacy of SoCraTes. This is where I first heard the Maker (Adding) vs Mender (Fixing) distinction. Scott added a perpendicular attribute of Hacker (Speed) vs Crafter (Quality). The two distinctions create 2x2 matrix defining four different types of developers: Rapid Prototyper (Maker/Hacker), Feature Builder (Maker/Crafter), Emergency Responder (Mender/Hacker) and Software Remodeler (Mender/Crafter). Scott then lists the Project Traits and People Traits that best fit each quadrant.

...
# Introduction
I've struggled to make any meaningful progress with this blog entry.
I'm finding it difficult to construct a good narrative.

Since I have no practical or professional experience with AI, I'm not sure I have any meaningful credibility to discuss how it will shape software engineering.
The field is changing rapidly.
I know a change is coming.
I just don't know what that change will be or what form it will take.
I doubt that anyone knows how AI will shape software engineering, even those who are using AI practically and professionally.
All I can do is state my opinions and impressions.

___Caveat Emptor.___

Given that my opinions and impressions are based upon external observations of a constantly changing field, I think the only way I can make progress and publish this is to do the following:
* List my opinions and impressions as they currently stand.
* Structure this blog entry as an anthology of related themes rather than one cohesive story. Treat each section as its own vinette much like the Jerry Lewis movie: [The Bellboy](https://en.wikipedia.org/wiki/The_Bellboy).
* Revisit this blog entry and update it as my opinions and impressions change, there are new advancements and discoveries, and there are new references to add.

With that preamble, let's start.

# The End of Coding as Toil
In my previous blog, [The End of Coding as Toil](https://jhumelsine.github.io/2026/05/29/code-and-toil.html), I discussed how Generative AI is changing software development and the role of software engineers.
I want to continue that theme.

I do not believe software engineering is disappearing.
I think the role of software engineering is changing.

The toil of hand written code is decreasing through AI.
However, the architecture, composition, boundaries, communication and judgment by humans is increasing.
Humans and AI will increasingly work together to architect, design and implement software systems faster and better than either could do alone.

# AI Is a Power Tool
<img src="https://cdn2.picryl.com/photo/2011/07/11/master-sgt-abner-ramos-cuts-a-piece-2x4-of-lumber-c027b8-1024.jpg" alt="Master Sgt. Abner Ramos cuts a piece 2x4 of lumber, as part of his table and shelves for his room here at Forward Operating Base Nejrab, Afghanistan  July 11." title="Image Source: https://nara.getarchive.net/media/master-sgt-abner-ramos-cuts-a-piece-2x4-of-lumber-c027b8" align="right" width = "20%" style="padding: 10px;">
...
A power saw in the hands of knuckleheads allows them to saw their thumbs off quicker.

# Maker vs Mender
Developers tend to fall along a spectrum of:
* Makers (Adding Features): Those who care about delivering features to the customer/user as quickly as possible.
* Menders (Correcting/Refactoring Existing Feature Implementations): Those who care about the clarity and structure of the code.

Both are important.
AI will accelerate makers, but this should not come at the expense of the clarity and structure of the code desired by menders.

<img src="https://res.cloudinary.com/innoq/image/upload/v1/uploads-production/jp593jslta79pze5zgjxu4r7jlof" alt="Maker vs Mender/Hacker vs Crafter Matrix" title="Image Source: https://www.innoq.com/en/blog/2024/08/menders-fuer-legacy-code/" align="right" width = "50%" style="padding: 10px;">

I first heard the ___Maker vs Mender___ distinction in M. Scott Ford's [Makers vs Menders](https://www.youtube.com/watch?v=YW0BpCThjuM) presentation.
Scott aligned developers along a Makers/Menders index and then added a perpendicular index of Hacker (Speed) vs Crafter (Quality).
The two attributes create 2x2 matrix defining four different types of developers: 
* Rapid Prototyper (Maker/Hacker)
* Feature Builder (Maker/Crafter)
* Emergency Responder (Mender/Hacker)
* Software Remodeler (Mender/Crafter).

Scott then lists the Project Traits and People Traits that best fit each quadrant.

# Vibe Coding
[Vibe coding](https://jhumelsine.github.io/2026/05/29/code-and-toil.html#vibe-coding-and-the-monkeys-paw) has its place.
It can be useful for:
* Prototype implementations that demonstrate features to customers as the focus of discussions about desired behavior. Prototypes can also be used as working models for the development team.
* Personal projects, where maintainability, scalability and security may not be an issue.
* Code generation that's well defined and scoped. For example, adding `equals()` and `hashCode()` to a Java class.
* Small well scoped and bounded problems. I use prompting to review the code samples I provide in my blog entries.
* Generating tools to aid you in your day-to-day work. These will probably be scripts. Tool generation is a special case of the small well scoped and bounded problems bullet above.

Vibe coding is probably not the best practice for generating large projects with many users.

Vibe coding is similar to any other AI prompting.
The main difference is that you request code rather than a report, table, essay, story, poem etc.

# Agents
Agents are workers that do tasks on your behalf, often at your direction:
* Travel agents arrange and make reservations for trips on your behalf based upon where you want to go.
* Real estate agents act on behalf of a buyer or seller to facilitate a home sale.
* Insurance agents act on behalf of an insurance company to sell policies to customers.

Software agents act on our behalf to create code based upon our specification requests.
This sounds similar to Vibe Coding, but I think it's more than that.
Agentic Coding feels more aligned with more complex software projects.

I view Agentic Coding as something a bit more sophisticated than Vibe Coding since software agents tend to:
* Have specialized skills and constraints, not to dissimilar from human software developers
* Work in a team of other agents. For example, an architect agent, a coding agent, a testing agent, a refactoring agent, etc.
* Work iteratively with a human director in smaller chunks rather than produce an entire implementation in one step.
...
For example:
* an implementation agent might use [Test-Driven Development](https://jhumelsine.github.io/2024/07/15/tdd.html)
* a behavioral agent might validate requirements through [Behavior-Driven Development](https://jhumelsine.github.io/2024/08/08/bdd.html)
* a [mutation testing](https://jhumelsine.github.io/2025/03/28/mutation-testing.html) agent might evaluate the quality of the tests
* architecture agents might validate [boundaries and dependencies](https://jhumelsine.github.io/2023/11/03/hexagonal-architecture-dependencies-knowledge.html)
* complexity metrics might constrain maintainability

The agents would not only collaborate, but they could be organized in a system of checks and balances constraining each other to help ensure confidence in the code that's generated.

We might also want to limit or restrict direct agent interaction. Instead agents might interaict via the artifacts flowing between them:
* requirements
* tests
* implementations
* logs
...
# Knowledge vs Wisdom
AI possesses enormous amounts of knowledge, but [knowledge is not wisdom](https://jhumelsine.github.io/2026/05/29/code-and-toil.html#wisdom-vs-knowledge).

Knowledge is knowing the rules of the game.
Wisdom is understanding the [strategies and tactics](https://jhumelsine.github.io/2023/08/24/its-your-move.html) required to play the game well.

The rules of chess fit on a single sheet of paper.
The strategies and tactics of chess fill volumes.
...
This is probably more greatly magnified with agents.
Agents possess knowledge about coding.
Agents may even posses knowledge about the _wisdom_ topics I listed above.
However, they may not apply that wisdom unless directed to do so.

# Confidence
The challenge with any substantial development resides in how we reach a reasonable level of confidence in the code that's been created.
* Is it correct?
* Is it well designed?
* Can it be easily modified?

Can we trust code created by agents?
These are not new questions.
Long before AI, we asked, "Can we trust code created by humans?"

These are the same questions we have had with human software developers as individual contributors.

My motto is: **Trust, but Verify.**

Though we have used testing and other processes to confirm the software, much of the confidence we had in our softare depended upon on the trust and confidence we had with experienced developers.
AI is good, but it's not the same looking a human developer in the eye.
We could also keep up with human developed code through reviews.
However, AI will generate code at a faster rate than humans can review.

We will need processes that provide sufficient level of confidence on their own.
We can't depend upon AI to tell us the truth about the veracity of the code it writes.
It has a tendency to confabulate.
For example, it may decide that the best way to get all tests to pass is to remove all asserts.
But then again, this isn't necessarily a problem with AI.
Plenty of humans have gotten failing tests to pass using similar techniques.

**NEED TO EXPAND THIS. NEED TO TALK ABOUT HUMAN QA AND REVIEWERS.**

# Adapt What We Know
Confidence in our software is not a new phenomenon.
We take developers on their word that their code works.

We have practices and procedures to gain confidence, such as:
* Automated Testing
* CI/CD Pipelines
* Dedicated Testers
* Regression Testing
...
I suspect that most of these practices will translate to agentic practices as well, at least in the beginning.
...
### Team Leads
Software developers will become more like a tech lead, systems engineer or conductor overseeing a team of AI agents.
Agents will perform most of the duties that human Individual Contributors had performed before.
People won't write much code, possibly none, but they may review and correct generated code.
...
Orchestrating agents is different than being the team lead of human individual contributors.
There will be more interaction and discussion with people.
There may not be as much interaction with agents once they start working on a task.

A team of agents has a few attributes that aren't as convenient than with a team of individual contributors.
A team of agents can be more easily configured and scaled as needed.

Different problems may require different compositions of agents.
Different problems may have a different agents with different skills.
The orchestrator may also manage different sets of agents for different sets of problems.
...
https://x.com/plainionist/status/2065460998668972188 - How I Made My Brownfield Codebase AI-First

From Suril:
* https://github.com/awslabs/aidlc-workflows/
* https://github.com/awslabs/aidlc-workflows/blob/main/docs/writing-inputs/inputs-quickstart.md
...
* Bob has been using swarmforge to implement Hunt the Wumpus in different programming languages:
    * Clojure - https://github.com/unclebob/experiment-htw-clj-swarm
    * Go - https://github.com/unclebob/experiment-htw-go-swarm
    * Java - He's mentioned Java Wumpus, but I don't see a GitHub Repo for it yet.

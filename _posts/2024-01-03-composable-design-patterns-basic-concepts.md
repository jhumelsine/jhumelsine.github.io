...
|---------|---------|--------------|------------|---------------|------------------|
| **Proxy** | Controls access to another object, adding a level of indirection | Can defer creation (virtual proxy), control access (protection proxy), or add behavior (remote proxy) | Reduces resource usage; adds security or logging; can act as a stand-in for remote objects | Adds complexity; may introduce latency if misused | Lazy-loading large objects, access control, remote service calls |
| **Decorator** | Dynamically adds responsibilities to objects without modifying their code | Wraps objects in decorator classes; each adds behavior before/after delegating | Promotes flexibility; avoids subclass explosion | Many small classes can make code harder to follow | UI element styling, I/O stream enhancements, logging wrappers |
| **Chain of Responsibility** | Passes a request along a chain of handlers until one processes it | Each handler decides to process the request or pass it on | Decouples sender from receiver; flexible request handling | May lead to requests going unhandled; hard to debug long chains | Event handling, middleware pipelines, technical support escalation |
| **Composite** | Treats individual objects and object compositions uniformly | Tree structure where leaf and composite nodes share the same interface | Simplifies client code; easy to add new components | Can make the design overly general; harder to restrict certain compositions | GUI components, file systems, organization hierarchies |
| **Specification** | Encapsulates business rules as reusable, combinable objects | Combines simple specifications with logical operators (AND, OR, NOT) | Improves maintainability; supports dynamic rule composition | Can become complex with deeply nested combinations | Filtering in repositories, business rule validation, query building |
| **Interpreter** | Defines a grammar and uses classes to represent and evaluate language rules | Represents grammar elements as class hierarchies; uses `interpret` method | Easy to extend with new grammar rules; works well for small languages | Inefficient for complex languages; class explosion for large grammars | Scripting engines, expression evaluation, DSL processing |
| **Key Differences** | **Structural patterns** (Proxy, Decorator, Composite) focus on **object composition and structure**, **Behavioral patterns** (Chain of Responsibility, Interpreter, Specification) focus on **interaction and decision-making** | Proxy controls access, Decorator adds behavior, Chain passes requests, Composite manages hierarchies, Specification defines rules, Interpreter parses/evaluates | Choose based on whether you need access control, runtime extension, flexible routing, uniform hierarchy handling, rule composition, or grammar interpretation | Each pattern adds flexibility through composition but may add runtime overhead | — |

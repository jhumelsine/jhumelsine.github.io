...
We can view an inheritance tree as a type of delegation, where behaviors are executed via the inheritance chain from a child class up through its ancestor classes. Child class methods can override ancestor methods while still accessing the ancestor method, which can be done via a call to `super` in Java.
...
Walt Disney explains the Multiplane Camera in this [2-minute video](https://www.youtube.com/watch?v=kN-eCBAOw60).
...
DrinkOrder buildDrinkOrder(String ingredients) {
    ...
    for (String ingredient : ingredients.split()) { // I know that this needs a bit more to iterate.
...
Composable behavior may be more difficult to diagnose with Decorator. This is the first major design pattern where behavior has been distributed across several classes. While it should be simple to understand individual concrete classes in a Decorator design, the responsibility to orchestrate observable behavior resides in the `Configurer` in composing the objects from those classes correctly.

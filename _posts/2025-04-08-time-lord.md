...
`isMature(Order order)` delegates to `getNow()`, which being a __package-private__ method, we can override to return any value desired.  When not overridden, it will return the current system time.

...
We paired together adding test coverage to existing code. I was pleased with how much progress we were able to make. I felt he was understanding the process too as we were applying it.
...
When coverage is dictated by management, developers will focus upon coverage, possibly with tests that are low quality. The only thing worse than lack of testing is poor testing, which only creates the illusion of security.
...
# The Semaphore
We had covered a significant portion of the code, but we still had a block of uncovered code with a Java [`Semaphore`]( https://docs.oracle.com/javase%2F8%2Fdocs%2Fapi%2F%2F/java/util/concurrent/Semaphore.html) and its [`tryAcquire`]( https://docs.oracle.com/javase%2F8%2Fdocs%2Fapi%2F%2F/java/util/concurrent/Semaphore.html#tryAcquire--) method. It was deep in the implementation being used for concurrency management. Concurrency is difficult to implement correctly and challenging to test.
...
We only need to test how our code reacts to what `Semaphore` does.

## What Does a Semaphore Do?
Suril and I didn’t need to become experts on Java’s [`Semaphore`]( https://docs.oracle.com/javase%2F8%2Fdocs%2Fapi%2F%2F/java/util/concurrent/Semaphore.html). We just needed to understand the [`boolean tryAcquire(long timeout, TimeUnit unit) throws InterruptedException`](https://docs.oracle.com/javase%2F8%2Fdocs%2Fapi%2F%2F/java/util/concurrent/Semaphore.html#tryAcquire-long-java.util.concurrent.TimeUnit-) well enough to emulate its behavior.
...
However, our efficiency came at a cost. Our tests had implementation knowledge. They were brittle. But given how few and small we were willing to accept that price.

...
I'll highlight a few points starting with my basic Hexagonal Architecture diagram:
...
The Business Logic implementation will have references to these interfaces, because it's required to do so because the core domain behavior requires it.
...
`ManageStuff` has no dependency or knowledge upon either of these Frameworks or the External Dependencies. `ManageStuff` is loosely couplied to the Frameworks and Dependencies fit Adapters, which are created via the `Configurer`. More generally, none of the classes depend upon nor have knowledge of each other.
...
## Behavior and Depth
Many Hexagonal Architectures represent, or give the impression, that external frameworks and dependencies must be resources outside of the system, such as products provided by external vendors, open source, etc.
...
The name is rather long, but that's my main issue with it. It contains **And**. This method is doing more than one thing. We may break the DB functionality when adding the Message Service functionality. The Adapter has more than one reason to be updated. In the future it could be updated because of DB dependencies or Message Service dependencies. An update to one feature runs the risk of breaking the other feature. This **And** Adapter violates the [Single Responsibility Principle](https://en.wikipedia.org/wiki/Single-responsibility_principle) (SRP).
...
I worked on a project where one of the features stored Documents for people. These Documents could be Word documents, PDFs, etc. For years, they were stored in a traditional database as a blob of bytes, because that wast the only storage mechanism we had. The simplified diagram would look like this:
...
I added one invariant in the implementation above. If a Document is not found in Cloud Storage, but it is in the DB and the DB has been disabled, then return the DB's Document and notify someone that there's an inconsistency, and in a real world scenario, more context would be provided..
Other invariants may be desired, but I only provided this one as an example.
...
I often think about what I want to put into a blog while I drift to sleep in the evening. While dozing off I had a thought. My relationship lines leaving my Purple Hexagons always point outward. Cockburn doesn't address this at all. Martin gets it backwards.

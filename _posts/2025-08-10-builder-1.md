...
In this post, I’ll walk through the Builder pattern using the original Gang of Four (GoF) explanation and compare it with several modern interpretations. Whether you're revisiting Builder or learning it for the first time, you'll come away with a clearer understanding of where, when, and how to apply it.
...
The GoF featured a complex object in their sample code, which was a `MazeBuilder`. `MazeBuilder`, an abstract interface, allowed the client code to build a `Maze` by adding integer identified `Rooms` to the `Maze` and connecting pairs of rooms via a `Door`. `MazeBuilder` supported the construction of a `Maze` of any size or complexity from a single `Room` to thousands of `Rooms` with connecting `Doors`. Their `Maze` was a contextual model of an [undirected graph](https://en.wikipedia.org/wiki/Graph_theory#Graph).
...
I anticipate several Builder blog entries. I will continue with more Builder content in subsequent blog entries.

...
unlisted: true
---
__Add new diagram here.__ BOB THE BUILDER
...
What is a complex object? It’s an object with optional or repeating parts. The GoF featured a `MazeBuilder` in their sample code, which allowed the client code to build a `Maze` by adding integer identified `Rooms` to the `Maze` and connecting pairs of rooms via a `Door`. Their `Maze` was a simple [undirected graph](https://en.wikipedia.org/wiki/Graph_theory#Graph).

They defined an abstract `MazeBuilder`, which returns a built `Maze`. They define two concrete `MazeBuilders`, `StandardMazeBuilder` and `CountingMazeBuilder`.

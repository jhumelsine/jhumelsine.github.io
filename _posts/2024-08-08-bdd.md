...
__BDD tests specify behavior for the [Software Under Test](https://en.wikipedia.org/wiki/System_under_test) (SUT), whose implementation is challenged and confirmed each time the tests are executed.__ Behavior specification in the test and behavior observation from the implementation consistency is confirmed each time the tests are executed. Bob Martin uses [Double-Entry Bookkeeping](https://blog.cleancoder.com/uncle-bob/2017/12/18/Excuses.html) as an analogy.

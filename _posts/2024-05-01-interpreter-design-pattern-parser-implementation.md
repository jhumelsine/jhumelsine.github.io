...
My Rational Expression Evaluator Domain-Specific Language (DSL) has simple tokens. They are integers, alphanumerics and several symbols. I think I can hand-craft something simple.
...
* `toString()` returns the state of the `Scanner`.
...
* All subsequent calls to `peek(int)` and `consume()` should throw an exception once all tokens are consumed, since there are no more tokens to view or consume. The Out of
* Bounds exception is a little leaky, but I’m going to use it for now just to keep the implementation smaller.
...
* I’ve kept the `Scanner` implementation bare bones. If I were designing a DSL for production use:
    * I would have kept track of line and column information in the input so that when a syntax error is encountered, it would indicate the first unexpected character with context.
...
* My implementation is a form of state machine. It’s evaluating the string one symbol at a time as it processes them be creating tokens.
...
# Parser
The Parser implementation will more closely follow the techniques I specified in [Scanner and Parser]( https://jhumelsine.github.io/2024/04/25/interpreter-design-pattern-parser.html). The implementation progress will basically mirror the same phases in the [Design to Implementation](https://jhumelsine.github.io/2024/04/17/interpreter-design-pattern-implementation.html) blog.

__OR__ and __AND__ concepts are very important in grammar, design, implementation and the parser. It’s the concept that binds them together. Define a solid grammar using __OR__ and __AND__ rules and the design, implementation and parser practically write themselves.
* __OR__ rules, which:
    * Map to IS-A relationships in the design.
    * Map to interfaces and inheritance in the implementation.
    * Map to a method that peeks ahead at tokens to determine which of several more specific rule methods to call recursively.
* __AND__ rules, which:
    * Map to HAS-A relationships in the design.
    * Map to field attributes in the implementation.
    * Map to a method that consumes tokens, recursively call the rule methods recursively that define the rule and assemble all elements into a __AND__ object.

The `Parser` implementation will basically mirror the steps that were taken for the [class implementation](https://jhumelsine.github.io/2024/04/17/interpreter-design-pattern-implementation.html). Even the tests will basically be the same, except now they will be confirming the `Parser`’s ability to create and construct to the class objects rather than the functionality of the classes themselves.
...
It will map to a `Parser` test by removing the specific `Rational` reference and replacing it with a `Parser` reference as so:
```java
...
Here is the initial `Parser` implementation with just enough non-hardcoded content to allow this test to pass. That is, it doesn’t hardcode something like: `return new Rational(“0”);`; although, I did do that as a stepping stone to this implementation.

Look at the parallels of these methods and the [Design to Implementation – Where to Start?](https://jhumelsine.github.io/2024/04/17/interpreter-design-pattern-implementation.html#where-to-start). I’ll add more tests to cover more scenarios and their corresponding implementations.
```java
class Parser {
...
Expression is an __OR__ rule. The `Parser` needs to identify which rule to descend to for additional processing. Right now, the implementation supports `Rational` by default, and I’m adding `SubtractOperation`.
...
Here’s what the `Parser` based equivalent tests look like:
```java
...
I have one more follow up blog, but it will be feature the DSL I designed and implemented in my career and some of the experienced our team had with the pattern.
...
class Parser {
    ...
    // OR Rule. Determine which specific rule to apply.
    ...
    // AND Rule. Consume tokens and build object from its components.
    private DivideOp parseDivideOp() throws Exception {
        Token slashToken = scanner.consume();

        if (!slashToken.isSlash()) throw new Exception("Expected Slash Symbol (/). Got: " + slashToken);
    ...
    private AddOp parseAddOp() throws Exception {
        ...
        if (!plusToken.isPlus()) throw new Exception("Expected Plus Symbol (+). Got: " + plusToken);
    ...
    // AND Rule. Consume tokens and build object from its components.
    private MultiplyOp parseMultiplyOp() throws Exception {
        Token starToken = scanner.consume();

        if (!starToken.isStar()) throw new Exception("Expected Star Symbol (*). Got: " + starToken);
        ...
        while (true) {
            multiplyOp.addExpression(parseExpression());

...
# The Spoiler
If you made it through my previous blogs, you’ll see a definite progression of __Domain-Specific Language__ (DSL) to __Grammar__ to __Design__ to __Implementation__ to __Scanner/Parser__.
...
# The Domain
I’m going to feature a domain that wasn’t my project’s actual domain. They are conceptually similar, but the blog domain should be more familiar to a wider audience.

Imagine an automated package processing center, such as one used by [United Parcel Service]( https://www.ups.com), [Federal Express]( https://www.fedex.com/) or [DHL](https://www.dhl.com/). Packages enter the processing center at an inbound loading dock. They’re routed through the center based upon address, priority, client loyalty program, etc. with their final destination being an outbound loading dock.
...
* Is Local Address (or its negation)
* Route to Immediate Delivery Local Ground Transportation
* Route to Air Transportation
* Route to Filled-Truck Ground Transportation
* Etc.
...
action lowPriorityRouting = {
    if (isLocal()) {
        RouteLocalGroundTransportation(Local, Filled)
    } else {
...
ID ::= alphanumeric
CONDITION ::= conditionId | AND_CONDITION | OR_CONDITION | NOT_CONDITION
...
ACTION ::= actionId | CONDITION_ACTION | ACTION_LIST
...
# The Design
The design I had sketched on the whiteboard was close, but it needed a few elements for the Parser, specifically how to resolve the Pseudo Keywords, `Action`/`Command` identifiers, and Decomposed Packages listed above.
...
3.	The Repo searches for an object in its collection that matches the name. It returns the `PrototypeCondition` returned from the object’s `copy()` method.
...
We leaned upon the itinerary greatly after that. It made it trivial for us to confirm that `Package`s were being routed as we expected. Any unexpected routing was quickly found and addressed.
...
We were able to experiment with Driver based Routing Rules for Android relatively easily.

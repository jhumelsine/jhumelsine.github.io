...
I think there's also a third one, more fundamental, which I'll present in the next blog.
...
Previous automated testing experiences weren't fruitful. Here are some, which I may go into more detail with these in a future blog:
...
1.	__Write a failing test case.__  Since there is no implementation for that scenario, it should fail. 
2.	__Make the failing test case pass.__ Hard-code a passing solution, even if it doesn’t implement the general case. This is the _Make it work_ phase. Bob Martin refers to this as: _Fake it until you make it._ This made no sense to me when I first read it. Shouldn’t we implement the solution?
3.	__Refactor.__ Clean up the code. This is the _Make it right_ phase. Remove hard-coded return values. Make it more general. You have the confidence to do this, because you have passing test cases, which will alert you if you break something while refactoring. 
4.	__Repeat.__ Add more specific test cases and as you refactor the code, it will transform into a general solution. Add tests until you can't another failing test. _As the tests get more specific, the code gets more generic._ – [Bob Martin](https://blog.cleancoder.com/uncle-bob/2014/12/17/TheCyclesOfTDD.html).
...
A complete __1-2-3__ cycle is often called the ___Red-Green-Blue___ TDD cycle. Red for the failing test. Green for the passing test. Blue for refactoring. Step 4 repeates the ___Red-Green-Blue___ cycle.
...
Each ___Red-Green-Blue___ cycle should only take about 30 to 120 seconds with 3TDD. Multiple iterations may be needed until one unit test and its implementation are complete.
...
Period. The code is always working. If anything fails, it vanishes, and you're back to your previous passing green state. You're always in a known state, but it’s quite Draconian. If a test fails, then poof, all non-committed work vanishes.
...
I feel TCR is mostly consistent with 3TDD. It adds version management to the 3TDD process. There is one aspect of TCR that I don’t like. There’s no ability to introduce the failing test, which I think is critical to the overall process. I don’t know how TCR prevents false positive tests.
...
Sometimes it’s painful to see updates I added an hour ago vanish. But once I start again, I run my test suite more frequently. I also find that I proceed much faster on the second attempt and without any further issues.
...
# Summary
I’m going to wrap it up here, but there’s more to come. I’ll continue with TDD themes with a true story, how to determine what to test and some of the benefits of TDD and possibly more.
...
* TDD:
    * __Test Driven Development: By Example__ by Kent Beck ([O’Reily](https://learning.oreilly.com/library/view/test-driven-development/0321146530/), [Amazon](https://www.amazon.com/Test-Driven-Development-Kent-Beck/dp/0321146530/))
    ...
    * Bob Martin's TDD related Clean Coder videos (NOTE: Behind paywalls):
        ...
        * Advanced TDD‚ Part 2: [Clean Coders](https://cleancoders.com/episode/clean-code-episode-19-p2), [O’Reilly](https://learning.oreilly.com/videos/clean-code-fundamentals/9780134661742/9780134661742-code_02_19_02/). Bob shows how the tests can drive the development.
        ...
        * Transformation Priority Premise‚ Part 1: [Clean Coders](https://cleancoders.com/episode/clean-code-episode-24-p1), [O’Reilly](https://learning.oreilly.com/videos/clean-code-fundamentals/9780134661742/9780134661742-code_02_24_01/). [Transformation Priority Premise](https://en.wikipedia.org/wiki/Transformation_Priority_Premise) (TPP) is a set of transformation rules Bob has cateloged to apply to the implementation to make it more generic.

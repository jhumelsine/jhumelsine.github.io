...
Previous automated testing experiences weren't fruitful. Here are some common experiences or impressions, which I may go into more detail with these in a future blog:
...
Step #3 updates the implementation making it more general while the tests provide the safety net. Be careful to only refactor existing behavior covered by tests. Don’t implement new behavior that doesn’t have a test case that specifies it.
...
A complete __1-2-3__ cycle is often called the ___Red-Green-Blue TDD___ cycle. Red for the failing test. Green for the passing test. Blue for refactoring. Step 4 repeats the ___Red-Green-Blue___ cycle.

Step #4 requires creative thinking and domain knowledge. We want to specify all scenarios based upon customer/domain/behavior needs. We don’t want tests based upon the implementation. I’ll address this in a future blog about behavior.
...
Repeat with a new test using the __Red-Green-Blue TDD__ process until you can no longer create a failing test.
...
## Refactoring
Returning to _Make it work; make it right_, briefly. Developers often get their code to work. But they often don’t take the time to make it right. Code is like any creative writing endeavor. The first draft puts your initial ideas on paper. Refinement makes it comprehensible by others.

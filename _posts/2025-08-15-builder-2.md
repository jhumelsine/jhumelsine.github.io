...
There’s a bit more to this implementation, so each developer or team will need to decide if it's worth the investment. Howeer, it ensures that `Pizza` can only be constructed via the `Pizza.Builder` and once built, the `Pizza` instance is immutable.

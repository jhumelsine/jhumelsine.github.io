...
While it's not an issue with `Pizza` toppings, when using a mechanism like this, we have to ensure the default behavior for any element not set, such as toppings for `Pizza`, still makes sense. A `Pizza` without toppings is a plain cheese pizza.

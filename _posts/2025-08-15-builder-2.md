...
There’s a bit more to this implementation, but if includes several of the attributes of the Builder pattern from the previous blog (TBD):
* 

but it ensures that `Pizza` can only be constructed via the `Pizza.Builder` and once built, it’s immutable.

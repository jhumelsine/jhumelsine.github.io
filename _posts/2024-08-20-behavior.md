...
These conversations should distill into [Acceptance Criteria](https://agileforgrowth.com/blog/acceptance-criteria-checklist/) (AC) which specify observable behaviors in the __Given-When-Then__ structure. ACs may generate more questions as well.
...
This test isn’t so much about functional behavior as it is about UI/UX behavior. `UserRegistrationButton` is an [Adapter](https://jhumelsine.github.io/2023/09/29/adapter-design-pattern.html). It implements `Button` by delegating to `UserRegistration`. This __User Registration__ to happen at the press of a `Button` without `UserRegistration` knowing about or depending upon the `Button`. The design also supports additional __Adapters__ if needed for a Mobile App or Alexa without any updates required for `UserRegistration`.

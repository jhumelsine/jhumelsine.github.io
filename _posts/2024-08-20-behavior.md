...
I still don’t know any `DB` details. This is an architectural decision that’s been isolated from the rest of the design, so it can be delayed. The `DB` could be Mongo, MS-SQL or even a flat file. The dependency details would be managed in `UserRepoAdapter`. Each potential DB would have its own dedicated Adapter, such as `UserRepoMongoAdapter`, `UserRepoMsSQLAdapter` or `UserRepoFlatFileAdapter`. For example, we might initially deploy with a `UserRepoFlatFileAdapter` and as the product matured, move to a production DB swapping out the previous Adapter with `UserRepoMongoAdapter`.
